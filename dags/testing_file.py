...
create_schema = PostgresOperator(
    ...
    sql=f"""
    CREATE SCHEMA IF NOT EXISTS {TABLE_NAME};
    """,
    dag=dag,
)
...
def load_fhvhv_tripdata_table_from_minio():
    ...
    drop_table_sql = f"""
    DROP TABLE IF EXISTS {TABLE_NAME}.fhvhv_2024;
    """
    ...
    create_table_sql = f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME}.fhvhv_2024 (
    ...
    for i in range(parquet_file.num_row_groups):
        ...
        copy_sql = f"COPY {TABLE_NAME}.fhvhv_2024 FROM STDIN WITH (FORMAT CSV)"
...
def load_fhv_tripdata_table_from_minio():
    ...
    # --- Drop and recreate table ---
    drop_table_sql = f"""
    DROP TABLE IF EXISTS {TABLE_NAME}.fhv_2024;
    """
    pg_hook.run(drop_table_sql)
    ...
    create_table_sql = f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME}.fhv_2024 (
        dispatching_base_num VARCHAR,
        pickup_datetime TIMESTAMP,
        dropoff_datetime TIMESTAMP,
        PUlocationID INT,
    ...
    for i in range(parquet_file.num_row_groups):
        ...
        copy_sql = f"COPY {TABLE_NAME}.fhv_2024 FROM STDIN WITH (FORMAT CSV)"
        cursor.copy_expert(copy_sql, buffer)
        conn.commit()
...
def load_yellow_tripdata_table_from_minio():
    ...
    # --- Drop and recreate table ---
    drop_table_sql = f"""
    DROP TABLE IF EXISTS {TABLE_NAME}.yellow_2024;
    """
    pg_hook.run(drop_table_sql)
    ...
    create_table_sql = f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME}.yellow_2024 (
        VendorID INTEGER,
    ...
    for i in range(parquet_file.num_row_groups):
        ...
        copy_sql = f"COPY {TABLE_NAME}.yellow_2024 FROM STDIN WITH (FORMAT CSV)"
        cursor.copy_expert(copy_sql, buffer)
        conn.commit()
...
def load_green_tripdata_table_from_minio():
    ...
    # --- Drop and recreate table ---
    drop_table_sql = f"""
    DROP TABLE IF EXISTS {TABLE_NAME}.green_2024;
    """
    pg_hook.run(drop_table_sql)
    ...
    create_table_sql = f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME}.green_2024 (
        VendorID INT,
    ...
    for i in range(parquet_file.num_row_groups):
        ...
        copy_sql = f"COPY {TABLE_NAME}.green_2024 FROM STDIN WITH (FORMAT CSV)"
        cursor.copy_expert(copy_sql, buffer)
        conn.commit()
